...
<section class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm" @if ($editable) id="po-terms-section" @endif>
    <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
            ...
            @if ($editable)
                <p class="mt-2 text-xs text-slate-500">General terms are captured from the library when you save; show and print use that saved snapshot (like RFQs). While editing, the preview below reflects the current library and linked P.R. scope. Scope terms from linked P.R. lines and maintenance/dismantling fields are included automatically.</p>
            @endif
        </div>
