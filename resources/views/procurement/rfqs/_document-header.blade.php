@php
    ...
    if (! isset($company) && isset($rfq)) {
        $procurementRequest = $rfq->items->first()?->procurementRequestItem?->procurementRequest;
        $company = PrCompany::resolve($procurementRequest?->company_key);
    }
...
<table class="w-full border-2 border-slate-900 border-collapse text-slate-900">
    <tr>
        ...
        <td class="border border-slate-900 p-4 text-center align-middle">
            <p class="text-xl font-bold tracking-tight sm:text-2xl">Request for Quotation</p>
        </td>
    </tr>
</table>
