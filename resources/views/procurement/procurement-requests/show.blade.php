    ...
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between print:hidden">
        ...
        <div class="flex flex-wrap gap-3">
            @if (auth()->user()->hasPermission('procurement-requests.update'))
                <a href="{{ route('procurement-requests.edit', $procurementRequest) }}" class="rounded-lg bg-brand px-4 py-2 text-sm font-medium text-white hover:bg-brand-hover">Edit</a>
            @endif
        ...
        <section class="rounded-xl border border-slate-200 bg-white p-6 text-sm shadow-sm">
            ...
            <div class="mt-4 overflow-x-auto">
                <table class="min-w-full text-left text-sm">
                    <thead class="text-xs uppercase text-slate-500"><tr><th class="py-2 pr-3">Item</th><th class="py-2 pr-3">Zone</th><th class="py-2 pr-3">Category</th><th class="py-2 pr-3">Subcategory</th><th class="py-2 pr-3">Description</th><th class="py-2 pr-3">Qty</th><th class="py-2 pr-3">Unit</th><th class="py-2 pr-3">Unit price</th><th class="py-2">Total</th></tr></thead>
                    ...
                    @forelse ($procurementRequest->items as $line)
                        <tr>
                            ...
                            <td class="py-2 pr-3">{{ $line->resolvedCategoryName() ?: '—' }}</td>
                            <td class="py-2 pr-3">{{ $line->resolvedSubcategoryName() ?: '—' }}</td>
                    ...
                    @empty
                        <tr><td colspan="9" class="py-4 text-slate-500">No BOQ lines.</td></tr>
                    @endforelse
                    </tbody>
                </table>
            </div>
