@php
    $bank = $buyer['bank'] ?? ['title' => $printLabels->t('bank_title'), 'lines' => []];
    if (! isset($bank['title']) || $bank['title'] === 'معلومات البنك') {
        $bank['title'] = $printLabels->t('bank_title');
    }
...
@if (count($bankLines) > 0)
    <div class="inv-bank-block">
        <div class="inv-bank-title">{{ $bank['title'] }}</div>
